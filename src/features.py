...
class CraigslistFeatureEngineer(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X: pd.DataFrame, y: Any = None) -> "CraigslistFeatureEngineer":
        df = self._to_frame(X)
    ...
    def _extract_price_series(self, *, y: Any, index: pd.Index) -> pd.Series:
        if y is None:
            raise ValueError("CraigslistFeatureEngineer.fit requires y=price.")
    ...
    def _to_frame(X: pd.DataFrame) -> pd.DataFrame:
        if not isinstance(X, pd.DataFrame):
            raise TypeError("CraigslistFeatureEngineer expects pandas.DataFrame input.")
        return X.copy()
